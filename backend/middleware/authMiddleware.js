...
async function confirmUserAccess(req, res, next) {
    try {
        let document;
        if (req.baseUrl.includes('Drink')) { 
            document = await Drink.findOne({ _id: req.params.id, user: req.user })
        } else {
            document = await Favorite.findOne({ _id: req.params.id, user: req.user })
        }
        if (!document) {
